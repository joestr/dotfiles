...
joehaste () {
	...
	if (( $# == 0 )) && [[ $(printf "%s" "$0" | wc -c) > 0 ]]
		...
		then
		echo "Usage: $0 FILE"
		echo "Upload contents of plaintext document to hastebin instance running on https://paste.joestr.xyz."
	...
	output=$(curl -# -f -XPOST "https://paste.joestr.xyz/documents" -d"$contents")
	if (( $? == 0 )) && [[ $output =~ \"key\" ]]
		then
		returnfile=$(sed 's/^.*"key":"/https:\/\/paste.joestr.xyz\//;s/".*$//' <<< "$output")
		if [[ -n $returnfile ]]
			then
			echo "$returnfile"
			return 0
